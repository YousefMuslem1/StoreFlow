...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('dataentry.getproducts') }}">جميع المنتجات</a></li>
        ...
        <div class="col-sm-4">
            <form action="" class="form-inline">
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" id="set_price_ounce" value="" class="form-control">
                </div>
    ...
    <div class="row">
        <div class="col-md-8">
            <form action="{{ route('dataentry.store') }}" method="post" id="createProductForm">
                @csrf
                <div class="row mb-2">
                    <div class="col-sm-6 col-md-3">
                        <div class="form-check d-none">
                            <input class="form-check-input" type="checkbox" name="quantity_check_box"
                                {{ old('quantity_check_box') ? 'checked' : '' }} id="quantity_selected" checked>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="caliber" id="caliber"
                                class="type-select custom-select {{ $errors->has('caliber') ? ' is-invalid' : '' }}"
                                value="{{ old('caliber') }}">
                                <option value="">إختر من هنا</option>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="ounce_price">سعر التحويل</label>
                            <input type="number" step="0.01" id="ounce_price" name="ounce_price" value=""
                                class="form-control">
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            if (Cookies.get('caliber')) {
                $('#caliber').val(Cookies.get('caliber')).trigger(
                'change');
            }
            if (Cookies.get('type')) {
                $('#type').val(Cookies.get('type')).trigger(
                'change');
            }
            if (Cookies.get('quantity')) {
                $('#quantity_select').val(Cookies.get('quantity')).trigger(
                'change'); // Ensure select2 updates the displayed value
            }
            ...
            $('#createProductForm').on('submit', function() {
                ...
                let selectedQuantity = $('#quantity_select').val();
                if (selectedCaliber) {
                ...
                if (selectedType) {
                    ...
                    if (selectedQuantity) {
                        Cookies.set('quantity', selectedQuantity, {
                            expires: 7
                        }); // Expires in 7 days
                    }
                }
