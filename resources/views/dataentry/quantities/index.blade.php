            ...
            <tbody>
                @foreach ($quantities as $quantity)
                    <tr>
                        ...
                        <td class="d-flex">
                            <button class="btn btn-success btn-sm AddQuantityBtn ml-1" id="AddQuantity" data-toggle="modal"
                        ...
                        "
                            ...
                            <button class="btn btn-danger btn-sm editQuantityBtn" data-target="#EditQuantityModal"
                            ...
                            <button class="btn btn-secondary btn-sm transfareQuantityBtn ml-1" data-toggle="modal"
                                data-target="#TransfareQuantityModal" data-short-ident="{{ $quantity->short_ident }}"><i
                                    class="fas fa-exchange-alt"></i></button>
                            <a href="{{ route('quantities.entry.quantity_details', $quantity->id) }}" target="_blank"
        ...
        $(document).ready(function() {
            ...
            $('#AddproductShortIdent').on('input', function() {
                ...
                if (product.trim() !== '') {
                    ...
                    $.ajax({
                        url: '/quantities/entry/get_product_info/' +
                            product, // Replace with your route
                        method: 'GET',
                        data: {
                            product,
                            _token: "{{ csrf_token() }}"
            ...
            $("#AddQuantitySubmit").click(function() {
                $('#AddQuantitySubmit').prop('disabled', true)
                ...
                if (status == 4) {
                    $.ajax({
                        url: "/quantities/entry/add_to_existing_quantity",
            ...
            $("#EditQuantitySubmit").click(function() {
                ...
                if (status == 1) {
                    $.ajax({
                        url: "{{ route('quantities.entry.sell_quantity') }}",
                        method: "POST",
                        data: {
                            short_ident,
                            status,
            ...
            $("#TransfareQuantitySubmit").click(function() {
                ...
                $.ajax({
                    url: "{{ route('quantities.entry.transfareQuantity') }}",
                    method: "POST",
                    data: {
                        short_ident, // quantity short_ident
                        TransfareWeight,
